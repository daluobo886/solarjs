...
// Minimal OrbitControls tailored for this demo (no dependencies outside local three.module.js)
class OrbitControls {
...
function createNoiseCanvas(size = 512, colors = ['#ffffff'], alpha = 0.35, horizontalBias = false, contrast = 0.12) {
  ...
  const gradient = ctx.createLinearGradient(0, 0, horizontalBias ? size : 0, horizontalBias ? 0 : size);
  colors.forEach((col, i) => gradient.addColorStop(i / (colors.length - 1 || 1), col));
...
function createStarfieldTexture(size = 1024, density = 0.0008) {
  const c = document.createElement('canvas');
  c.width = c.height = size;
  const ctx = c.getContext('2d');
  ...
  for (let i = 0; i < starCount; i++) {
    ...
    ctx.fillStyle = `hsla(${hue}, 70%, ${60 + Math.random() * 30}%, ${0.3 + Math.random() * 0.7})`;
    ctx.beginPath();
...
function createOrbitLine(radius, tilt = 0) {
  ...
  geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
  const material = new THREE.LineBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.5 });
...
function addMoon(hostName, { name, radius, distance, speed, color }) {
  ...
  const mat = createPlanetMaterial([color, '#d9d9d9', '#b0b0b0'], { roughness: 0.7 });
...
if (saturnSystem) {
  const ringTexture = createNoiseCanvas(1024, ['rgba(255,255,255,0.85)', 'rgba(189,170,140,0.65)', 'rgba(140,120,100,0.4)'], 0.4, true);
  const ringGeometry = new THREE.RingGeometry(saturnSystem.def.radius * 1.2, saturnSystem.def.radius * 1.85, 128, 1, 0, Math.PI * 2);
...
function focusOnBody(name) {
  ...
  if (name === 'Sun') {
    ...
    infoTextEl.innerHTML = `距离太阳：${planet.def.distance} AU（示意）<br>相对公转周期：${planet.def.speed.toFixed(2)}x`;    
  }

  flyTween = { startTime: performance.now(), duration, startPos, startTarget, targetPos, targetLook };
}
